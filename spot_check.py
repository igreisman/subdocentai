...
spot_checks = [
    ...
    ("How could a submarine run underwater without air?", "pam_007"),
    ("Why did submarines stay on the surface at night?", "pam_008"),
...
print(f"\n{'RESULT':<6} {'GOT':<12} {'EXP':<12} QUESTION")
print("-"*80)
pass_c = fail_c = 0
for q, exp in spot_checks:
    r = ask(q)
    got = r.get("faq_id") or (r.get("citations") or [{}])[0].get("chunk_id","??")
    ok = got == exp
    pass_c += ok; fail_c += not ok
    print(f"{'PASS' if ok else 'FAIL':<6} {got:<12} {exp:<12} {q[:55]}")
